...
enum CycleDerivation {
    ...
    static func window(point: Date, halfWidth: Double) -> DateInterval {
        DateInterval(
            start: DayMath.add(days: -halfWidth, to: point),
...
public struct Naive28Predictor: CyclePredictor {
    public init() {}
    public func predict(history: [PeriodEvent], profile _: UserProfile, asOf: Date) -> Prediction {
...
public struct BayesianCyclePredictor: CyclePredictor {
    ...
    private func makePrediction(
        ...
        let confidence: ConfidenceLevel = switch width {
        case ..<5: .high
        case ..<10: .moderate
        default: .low
        }
